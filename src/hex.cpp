...
hex::CubeNeighbors::CubeNeighbors(const glm::ivec3& start) {
  for (uint32_t i = 0; i < NEIGHBOR_COUNT; ++i) {
    neighbors[i] = start + CUBE_DIRECTIONS[i];
  }
}

hex::AxialNeighbors::AxialNeighbors(const glm::ivec2& start) {
  for (uint32_t i = 0; i < NEIGHBOR_COUNT; ++i) {
    neighbors[i] = start + AXIAL_DIRECTIONS[i];
  }
}
  
void hex::cube_neighbors(const glm::ivec3& start, std::vector<glm::ivec3>& neighbors) {
  neighbors.clear();
  for (uint32_t i = 0; i < NEIGHBOR_COUNT; ++i) {
...
glm::vec2 hex::axial_to_world(const glm::ivec2& axial_coord, uint32_t size) {
  const float x = size * SQRT_3 * (axial_coord.x + (axial_coord.y / 2.0f));
  const float y = size * (3.0f / 2.0f) * axial_coord.y;
  return glm::vec2(x, y);
}

glm::vec2 hex::cube_to_world(const glm::ivec3& cube_coord, uint32_t size) {
  return axial_to_world(cube_to_axial(cube_coord), size);
}

glm::vec2 hex::offset_to_world(const glm::ivec2& axial_coord, uint32_t size) {
...
glm::ivec2 hex::world_to_axial(const glm::vec2& world_coord, uint32_t size) {
...
glm::ivec3 hex::world_to_cube(const glm::vec2& world_coord, uint32_t size) {
  return axial_to_cube(world_to_axial(world_coord, size));
}

glm::ivec2 hex::world_to_offset(const glm::vec2& world_coord, uint32_t size) {
  return axial_to_offset(world_to_axial(world_coord, size));
}

// Rounding must maintain x + y + z = 0 
glm::ivec3 hex::cube_round(const glm::vec3& cube_coord) {
...
glm::ivec2 hex::axial_round(const glm::vec2& axial_coord) {
  return cube_to_axial(cube_round(axial_to_cube(axial_coord)));
}

uint32_t hex::cube_distance(const glm::ivec3& a, const glm::ivec3& b) {
  return std::max( { std::abs(a.x - b.x), std::abs(a.y - b.y), std::abs(a.z - b.z) } );
}

uint32_t hex::axial_distance(const glm::ivec2& a, const glm::ivec2& b) {
...
void hex::cubes_on_line(const glm::vec3& a, const glm::vec3& b, std::vector<glm::ivec3>& coords) {
